...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="translations" select="$translations_doc/rs/r[language=$language and (table='SBerStamm' or table='BerStammArt') and (column='Beschreibung' or column='Bezeichnung' or column='Art')]" />
	...
	<xsl:template match="/">
		<xsl:element name="tree">
			...
			<xsl:apply-templates select="$areatypes/rs/r" mode="appendNode">
				<xsl:sort select="bezeichnung_a" order="ascending" />
			</xsl:apply-templates>
			<xsl:apply-templates select="$areas/rs/r" mode="appendNode">
				<xsl:sort select="bezeichnung_a" order="ascending" />
			</xsl:apply-templates>
		</xsl:element>
	</xsl:template>

	<xsl:template match="r[not(not(email))]" mode="appendNode">
		<xsl:variable name="id" select="id" />
		<xsl:variable name="typeid" select="art" />
		...
		<xsl:if test="boolean(number($isReferenced))">
			<xsl:element name="item">
				...
				<xsl:element name="parents">
					<xsl:apply-templates select="$areauuids/rs/r[stammid=$id]" mode="unique" />
					...
					<xsl:apply-templates select="$areatree/rs/r[toid=$id]" mode="appendStructure">
	...
	<xsl:template match="r[not(email)]" mode="appendNode">
		<xsl:variable name="typeid" select="id" />
		<xsl:variable name="isReferenced">
			<xsl:apply-templates select="$areas/rs/r[art=$typeid]" mode="isReferenced" />
		</xsl:variable>
		<xsl:if test="boolean(number($isReferenced))">
			<xsl:apply-templates select="$areatypes/rs/r[id=$typeid]" mode="appendTypes">
				<xsl:sort select="art_a" order="ascending" />
				<xsl:with-param name="language" select="$language" />
				<xsl:with-param name="translations" select="$translations[table='BerStammArt']" />
				<xsl:with-param name="image" select="'./images/ui/areatype.png'" />
			</xsl:apply-templates>
		</xsl:if>
	</xsl:template>
